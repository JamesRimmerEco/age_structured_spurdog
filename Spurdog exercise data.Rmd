...
| `survival_prob` | Precomputed survival probability \( S = e^{-(M + F)} \). |
...
2. **How should fishing mortality be incorporated into the model?**  
   - Fishing is defined only between ages 5-30. Will you **extend** it if using **all age classes**, or **reduce** the matrix size?

3. **How do you convert mortality into survival probability?**  
   - This is **essential** for filling in the **sub-diagonal** of the Leslie matrix.

4. **How should fecundity be computed for the model?**  
